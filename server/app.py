...
def init_db():
    ...
    with conn() as db:
        db.execute(
            "CREATE TABLE IF NOT EXISTS posts(id INTEGER PRIMARY KEY AUTOINCREMENT,slug TEXT UNIQUE NOT NULL,category TEXT NOT NULL,title TEXT NOT NULL,excerpt TEXT NOT NULL,content TEXT NOT NULL,image TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'draft',created_at TEXT NOT NULL)"
        )
...
def serialize(row):
    return {
        ...
        "readTime": f"{max(2,len(' '.join(json.loads(row['content'])).split())//180+1)} Min.",
    }
...
def login():
    ...
    if not check_password_hash(os.environ["NELLOS_ADMIN_PASSWORD_HASH"], password):
...
def create_post():
    ...
    if (
        not title
        or category not in ("Fashion", "Cooking")
        or not excerpt
        or not body
    ...
    ext = Path(image.filename or "").suffix.lower() if image else ""
    if not image or ext not in (".jpg", ".jpeg", ".png", ".webp"):
    ...
    try:
        ...
        with Image.open(path) as im:
            ...
            im.save(path, optimize=True, quality=86)
    except Exception:
    ...
    with conn() as db:
        db.execute(
            "INSERT INTO posts(slug,category,title,excerpt,content,image,status,created_at) VALUES(?,?,?,?,?,?,?,?)",
            (
                slug,
                category,
                title,
...
def delete_post(post_id):
    ...
    if row["image"].startswith("/uploads/"):
        (UPLOAD_DIR / Path(row["image"]).name).unlink(missing_ok=True)
    return jsonify(ok=True)
